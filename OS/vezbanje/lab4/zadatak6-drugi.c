...
int main(int argc, char *argv[])
{
    ...
    struct mymsgbuf buf;
    strcpy(buf.mtext, "");
    pola2_file = fopen("pola2.txt", "a");
    do
    {
        // FIX: sizeof(buf.mtext) bez +1 -- msgrcv prima KAPACITET bafera, ne duzinu sadrzaja
        msgrcv(msgq_id, &buf, sizeof(buf.mtext), 2, 0);
        strcpy(line, buf.mtext);
        printf("Received message: %s", line);

        if (strcmp(line, "END") == 0) break;

        // dodato: konverzija u mala slova pre upisa (deo zadatka)
        for (int j = 0; line[j] != '\0'; j++)
            line[j] = tolower((unsigned char)line[j]);

        fputs(line, pola2_file);
    } while (strcmp(line, "END") != 0);
    printf("Received END indicator, ending execution\n");

    fclose(pola2_file); // dodato: zatvaranje fajla
